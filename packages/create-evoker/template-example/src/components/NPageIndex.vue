  ...
  <template v-else>
    ...
    <div class="flex flex-col mx-5 my-3">
      <div
        v-for="item of list"
        :key="item.id"
        class="flex flex-col bg-white dark:bg-black mb-2.5 rounded-sm relative"
      >
        <div
          class="flex justify-between items-center w-full h-14"
          :class="item.open ? 'text-gray-300' : 'text-black dark:text-white'"
          @click="onShowPageList(item)"
        >
          <span class="ml-3">{{ item.name }}</span>
          <img class="mr-3 w-6 h-6" :src="getSrc(item.id)" />
        </div>
        <div
          class="bg-white dark:bg-black rounded-b-sm relative overflow-hidden"
          :class="item.open ? 'h-auto' : 'h-0'"
        >
          <div
            class="flex flex-col duration-300 opacity-0 -translate-y-1/2"
            :class="item.open ? 'translate-y-0 opacity-100' : ''"
          >
            <template v-for="page of item.pages" :key="page.url">
              <div
                v-if="page.url.startsWith('@')"
                @click="showPopupComponent(page.url)"
                class="
                  flex
                  items-center
                  justify-between
...
const onShowPageList = (cur: PageGroup) => {
  const others = props.list.filter(item => {
